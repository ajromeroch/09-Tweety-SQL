...
router.get("/users/:username", function (req, res, next) {
	client.query(
		"SELECT * FROM tweets INNER JOIN users ON users.id = tweets.user_id WHERE users.name=$1",
		[req.params.username],
		(err, data) => {
			if (err) return next(err);
...
router.get("/tweets/:id", function (req, res, next) {
	client.query(
		"SELECT * FROM tweets INNER JOIN users ON users.id = tweets.user_id WHERE tweets.id = $1",
		[req.params.id],
		(err, data) => {
			if (err) return next(err);
...
router.post("/tweets", function (req, res, next) {
	...
	if (exist) {
		...
		//
		client.query(
			"INSERT INTO users (name, picture_url) VALUES ($1, 'http://i.imgur.com/JKInSVz.jpg');",
			[variables[0]],
			(err, data) => {
				if (err) return next(err);
			}
